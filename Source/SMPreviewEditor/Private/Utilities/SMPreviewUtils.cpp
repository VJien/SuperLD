...
struct FValidWorldCheck
{
	TWeakObjectPtr<UWorld> LastRepresentedWorld;
	UWorld* BadPointerCheck = nullptr;
	int32 TimesChecked = 0;
};

static TMap<FSMAdvancedPreviewScene*, FValidWorldCheck> ValidWorldChecks;
...
bool FSMPreviewOutlinerUtils::RefreshLevelEditorOutliner(FSMAdvancedPreviewScene* PreviewOwner)
{
	...
	if (LevelEditor.IsValid())
	{
		...
		{
			const SceneOutliner::FSharedOutlinerData& SharedData = SceneOutlinerPtr->GetSharedData();
			UWorld* RepresentingWorld = SharedData.RepresentingWorld;

			/*
			 * HACK:
			 * Attempt to check for an invalid RepresentingWorld pointer. The underlying engine code
			 * uses a raw pointer which can become stale temporarily after a map change.
			 *
			 * The pointer can't just be assigned to the TWeakObjectPtr, if it's invalid indexing may fail,
			 * although it works a surprising amount of the time.
			 */
			{
				FValidWorldCheck& WorldCheck = ValidWorldChecks.FindOrAdd(PreviewOwner);

				if (WorldCheck.LastRepresentedWorld.IsExplicitlyNull())
				{
					// This is the first run or we have explicitly set the pointer to null, and SHOULD be safe to assign.
					WorldCheck.LastRepresentedWorld = RepresentingWorld;
					WorldCheck.TimesChecked = 0;
					WorldCheck.BadPointerCheck = nullptr;
				}

				if (WorldCheck.LastRepresentedWorld.IsStale() || WorldCheck.LastRepresentedWorld.Get() != RepresentingWorld)
				{
					// We could be dealing with either a valid or state pointer, likely after a map change.
					
					if (WorldCheck.BadPointerCheck == nullptr)
					{
						WorldCheck.BadPointerCheck = RepresentingWorld;
					}
					else if (WorldCheck.BadPointerCheck != SharedData.RepresentingWorld || WorldCheck.TimesChecked++ >= 1)
					{
						// If the pointer has changed then we are very likely pointing at valid data. Invalid data shouldn't
						// last more than one frame. Times checked is still required in the event the pointer was always fine,
						// such as just starting/stopping PIE.
						WorldCheck.LastRepresentedWorld.Reset();
					}

					return false;
				}

				if (!WorldCheck.LastRepresentedWorld.IsValid())
				{
					// Couldn't set properly.
					return false;
				}
			}
			
			if (!SharedData.bRepresentingPlayWorld && IsValid(RepresentingWorld))
			{
...
void FSMPreviewOutlinerUtils::CleanupLevelEditorOutlinerChecks(FSMAdvancedPreviewScene* PreviewOwner)
{
	ValidWorldChecks.Remove(PreviewOwner);
}
